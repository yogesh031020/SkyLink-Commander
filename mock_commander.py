...
# Mock state simulating realistic drone flight data
drone_state = {
    "mode": "GUIDED", 
    "armed": True, 
    "alt": 0.00, 
        ...
        try:
            if climbing:
                ...
                if alt >= 12.54:
                    alt = 12.54
                    climbing = False
                    drone_state["status"] = "STABLE HOVER"
                    drone_state["mode"] = "ALT_HOLD"
            else:
                # Keep it hovering realistically around 12.5m
                alt = 12.54 + (time.time() % 0.2 - 0.1)
